...
// ========= CONFIG =========
const VERSION_TAG = 'vA-simple-id-only+timesheet-1';
...
function doPost(e) {
  try {
    ...
    if (action === 'create') {
      ...
      const evtTimesheet  = makeTimesheetEvent();  // monthly on 15 & 30, 09:00–17:00 WIB

      // Email with 2 GCal links + 2 ICS attachments
